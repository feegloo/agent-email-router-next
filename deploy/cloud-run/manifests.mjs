...
const containers = gpu ? [
  { name: 'gateway', image: `${REGISTRY}/gateway:${TAG}`, ports: [{ containerPort: 8080 }], env: env({ OLLAMA_MODEL: 'qwen3.5:0.8b' }), resources: { limits: { cpu: '1', memory: '512Mi' } }, startupProbe: tcp(8080), volumeMounts: [{ name: 'logs', mountPath: '/var/log/ollama' }] },
